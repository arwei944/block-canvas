...
export const SelectBlock: React.FC<SelectBlockProps> = ({ node }) => {
  ...
  const selectConfig = theme.components?.select || theme.components?.input || {};
  ...
  const selectStyle = useMemo<React.CSSProperties>(() => {
    const s: React.CSSProperties = {
      width: '100%',
      height: selectConfig.height || 36,
      padding: selectConfig.padding || '0 12px',
